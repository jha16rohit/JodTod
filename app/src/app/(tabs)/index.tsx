...
const moneyGive = [
  ...
  { name: 'Others', amount: '₹500', tag: '2 people', icon: 'ellipsis-horizontal' },
];
  ...
  return (
    <View className="flex-1">
      ...
      <Image
        ...
        resizeMode="cover"
      />
      {/* Soft wash so glass panels stay legible over busy background art */}
          ...
          <View className="mb-6 flex-row items-center justify-between">
            <View>
              <View className="flex-row items-center">
                <Text className="text-[17px] font-semibold text-[#0B3D62]">
                  {getGreeting()},
                </Text>
                <Text className="ml-1.5 text-[23px] font-extrabold text-[#0B3D62]">
                  {userName}
                </Text>
              </View>
              <Text className="mt-1 text-[11px] font-medium tracking-wide text-[#3E6E8E]">
                Split Smart. Stay Together.
              </Text>
            </View>

            {/* Header Actions — real glass buttons */}
            <View className="flex-row items-center gap-3">
              <View className="h-11 w-11 rounded-full" style={iconShadow}>
                <View className="h-full w-full overflow-hidden rounded-full border border-white/60">
                  <BlurView
                    intensity={50}
                    tint="light"
              ...
              <View className="relative h-11 w-11 rounded-full" style={iconShadow}>
                <View className="h-full w-full overflow-hidden rounded-full border border-white/60">
                  <BlurView
                    intensity={50}
                    tint="light"
                ...
                <View className="absolute right-2.5 top-2.5 h-2.5 w-2.5 rounded-full border-2 border-white bg-[#FF6548]" />
              </View>

              <View className="h-11 w-11 rounded-full" style={iconShadow}>
                <View className="h-full w-full overflow-hidden rounded-full border-2 border-white/70">
                  <BlurView
                    intensity={40}
                    tint="light"
                    style={[StyleSheet.absoluteFill, { borderRadius: 22 }]}
                  />
                  <View className="absolute inset-0 bg-[#6ED5B3]/30" />
<TouchableOpacity
                  activeOpacity={0.7}
                  onPress={() => router.push("/profile")}
                  className="h-full w-full items-center justify-center"
          ...
          <View className="mb-7 rounded-[32px]" style={heroShadow}>
            <View className="overflow-hidden rounded-[32px] border border-white/50">
              ...
              <LinearGradient
                colors={['rgba(28,120,148,0.82)', 'rgba(8,123,118,0.8)', 'rgba(24,168,110,0.82)']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
          ...
          >
            {groups.map((group, index) => (
              <View
                key={group.name}
                className={`mr-3 w-[178px] rounded-[20px] ${index === groups.length - 1 ? 'mr-0' : ''
                  }`}
                style={cardShadow}
              >
                  ...
                  <TouchableOpacity activeOpacity={0.9} className="p-3">
                    <View className="flex-row items-center">
                      <View className="mr-2.5 h-10 w-10 items-center justify-center rounded-full border border-white/60 bg-white/40">
                        <Ionicons name={group.icon as any} size={18} color="#0B3D62" />
                      </View>
                      <View className="flex-1">
                        <Text
                          numberOfLines={1}
                      ...
                      <Ionicons name="chevron-forward" size={16} color="#8DA0B1" />
                    </View>
                ...
                {expenses.map((expense, index) => (
                  ...
                  >
                    ...
                    <View className="ml-3 items-end">
                      ...
                      <Text
                        className={`text-[16px] font-extrabold ${expense.positive ? 'text-[#149C73]' : 'text-[#E84D39]'
                          }`}
                      >
                        {expense.amount}
                      </Text>
          ...
          <View className="mb-4 flex-row rounded-full border border-white/60 bg-white/40 p-1">
            <TouchableOpacity
              ...
              className={`flex-1 items-center rounded-full py-2.5 ${isGet ? 'bg-[#149C73]' : ''
                }`}
            >
              <Text
                className={`text-[13px] font-bold ${isGet ? 'text-white' : 'text-[#3E6E8E]'
                  }`}
              >
                You Get
              </Text>
            ...
            <TouchableOpacity
              ...
              className={`flex-1 items-center rounded-full py-2.5 ${!isGet ? 'bg-[#E84D39]' : ''
                }`}
            >
              <Text
                className={`text-[13px] font-bold ${!isGet ? 'text-white' : 'text-[#3E6E8E]'
            ...
            <View className="overflow-hidden rounded-[28px] border border-white/60">
              ...
              <View className="px-4">
                {flowRows.map((person, index) => (
                  ...
                  >
                    <View
                      className={`mr-3 h-9 w-1 rounded-full ${isGet ? 'bg-[#20B879]' : 'bg-[#F04F38]'
                        }`}
                    />
                    ...
                    <Text
                      className={`text-[16px] font-extrabold ${isGet ? 'text-[#149C73]' : 'text-[#E84D39]'
                        }`}
                    >
                      {isGet ? '+ ' : '- '}
                      {person.amount}
